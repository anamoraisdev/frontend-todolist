...
const FormCategorias = ({categorias, setCategorias, openCategorias, categoriaAEditar, setCategoriaAEditar}) => {
    ...
    return (
        ...
        <form onSubmit={(event) => aoSubmeterForm(event)} className=" flex flex-col items-start rounded-md gap-5 text-slate-700">
            ...
            <input type="text" placeholder="digite um nome para categoria" required value={nome} onChange={(event) => setNome(event.target.value)} className="rounded-lg p-2 dark:bg-indigo-300 dark:text-indigo-950 dark:placeholder-slate-500 focus:outline-1 focus:outline-indigo-400"></input>
